...
class WindowController {
  ...
  initialize ({ eventBus }) {
    ...
    this.eventBus.subscribe((event) => {
      ...
      const { criteria, options } = event.data;
      let pendingOperationPromise;
  ...
  update (definition, options = {}) {
    if (!this.db) {
      return Promise.reject('WindowController~update: The DB has not been initialized');
    }

    if (!definition?.id) {
      return Promise.reject('WindowController~update: No updates given. Cannot update item(s)');
    }

    const criteria = { id: definition.id };
    const newUpdates = definition;
